...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: task, error: fetchError } = await supabase
    .from("tasks")
    .select("id, assigned_to, admin_status, linked_to_task_id")
    .eq("id", id)
    .single()
  ...
  if (task.linked_to_task_id != null) {
    return NextResponse.json({ error: "Bağımlı göreve manuel süre eklenemez" }, { status: 400 })
  }
  ...
  const { data: entry, error: insertError } = await supabase
    .from("manual_time_entries")
    .insert({ task_id: parseInt(id, 10), user_id: user.id, hours, reason })
    .select()
    .single()
  ...
  await supabase.rpc("increment_manual_hours", {
    p_task_id: parseInt(id, 10),
    p_hours: hours,
  })
