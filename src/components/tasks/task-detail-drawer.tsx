...
import { Clock3, Link2, MapPin, Pencil, Timer, User2 } from "lucide-react"
...
function PersonValue({
  ...
  person?: { display_name: string; photo_url?: string | null } | null
  fallback: string
}) {
  if (!person) {
    return fallback
  ...
  return (
    <span className="inline-flex items-center justify-end gap-2">
      <UserAvatar displayName={person.display_name} photoUrl={person.photo_url} size="sm" />
      <span className="min-w-0 truncate">{person.display_name}</span>
    </span>
  )
}

function ApprovalValue({ task }: { task: Task }) {
  if (!task.approved_at) {
    return "-"
  }
  ...
  return (
    <span className="inline-flex flex-wrap items-center justify-end gap-2">
      <span>{formatDate(task.approved_at)}</span>
      {task.approved_by_user ? (
        <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-2 py-1 text-emerald-700">
          <UserAvatar
            displayName={task.approved_by_user.display_name}
            photoUrl={task.approved_by_user.photo_url}
            size="sm"
          />
          <span className="max-w-[160px] truncate text-xs font-semibold">{task.approved_by_user.display_name}</span>
        </span>
      ) : null}
    </span>
  )
...
export function TaskDetailDrawer({
  ...
  const manualSeconds = (task.manual_hours ?? 0) * 3600
  const totalSeconds = liveSeconds + manualSeconds
      ...
      <DialogContent className="left-auto right-0 top-0 flex h-screen max-w-[560px] translate-x-0 translate-y-0 flex-col overflow-hidden rounded-none border-l border-zinc-200 p-0 data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right">
        <DialogHeader className="flex-shrink-0 border-b border-zinc-100 px-6 py-5 pr-14 text-left">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <DialogTitle className="mt-2 truncate text-2xl font-semibold text-zinc-950">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-5">
          <section className="rounded-[24px] border border-zinc-200 bg-zinc-50/70 p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-4">
              ...
              <div className="rounded-2xl bg-white p-3">
                ...
                <p className="mt-1 text-base font-semibold text-zinc-950">{formatHours(manualSeconds)} sa</p>
              </div>
              <div className="rounded-2xl bg-white p-3">
                <p className="text-xs text-zinc-400">Toplam saat</p>
                <p className="mt-1 text-base font-semibold text-zinc-950">{formatHours(totalSeconds)} sa</p>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-[24px] border border-zinc-200 bg-white p-4">
            ...
            <InfoRow
              label="Atanan"
              value={<PersonValue person={task.assigned_user} fallback="Atanmamış" />}
            />
            <InfoRow
              label="Atayan"
              value={<PersonValue person={task.assigned_by_user} fallback="Atayan bilgisi yok" />}
            />
          ...
          <section className="rounded-[24px] border border-zinc-200 bg-white p-4">
            ...
            <InfoRow label="Onay" value={<ApprovalValue task={task} />} />
