...
export function ReportUserBreakdown({ reports }: ReportUserBreakdownProps) {
  return (
    <div className="space-y-3">
      {reports.map((report) => (
        <div key={report.user.id} className="rounded-xl border border-zinc-200 bg-white shadow-sm overflow-hidden">
            ...
            <TableBody>
              {report.tasks.map((task) => (
                <TableRow key={task.id} className="hover:bg-zinc-50/50">
                  ...
                  <TableCell className="text-right font-semibold font-mono text-sm">{getAdamSaat(task).toFixed(2)}</TableCell>
